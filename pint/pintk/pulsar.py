...
class Pulsar(object):
    ...
    def __init__(self, parfile=None, timfile=None, ephem=None):
        ...
        self.prefit_resids = pint.residuals.resids(self.selected_toas, self.prefit_model)
    ...
    def __getitem__(self, key):
        try:
            ...
            log.error('Parameter %s was not found in pulsar model %s' % (key, self.name))
            return None
    ...
    def add_phase_wrap(self, selected, phase):
        ...
        if 'pn' not in self.all_toas.table.colnames or 'pn' not in self.selected_toas.table.colnames:
            self.all_toas.compute_pulse_numbers(self.prefit_model)
            self.selected_toas.compute_pulse_numbers(self.prefit_model)
        if 'delta_pulse_number' not in self.all_toas.table.colnames or 'delta_pulse_number' not in self.selected_toas.table.colnames:
            self.all_toas.table['delta_pulse_number'] = np.zeros(len(self.all_toas.get_mjds()))
            self.selected_toas.table['delta_pulse_number'] = np.zeros(len(self.selected_toas.get_mjds()))
    ...
    def add_jump(self, selected):
        ...
        if "PhaseJump" not in self.prefit_model.components:
            ...
            for dict1, dict2 in zip(self.all_toas.table['flags'][selected], self.selected_toas.table['flags']):
                dict1['jump'] = 1
                dict2['jump'] = 1
            return param.name
        #if gets here, has at least one jump param already
        #if doesnt overlap or cancel, add the param
        jump_nums = [int(dict['jump']) if 'jump' in dict.keys() else np.nan for dict in self.all_toas.table['flags']]
        for num in range(1, int(np.nanmax(jump_nums)+1)):
                ...
                if np.array_equal(jump_select, selected):
                    ...
                    log.warn('The selected toa(s) overlap an existing jump. Remove all interfering jumps before attempting to jump these toas.')
                    return None
    ...
    def fit(self, selected, iters=1):
        ...
        self.postfit_resids = pint.residuals.resids(self.all_toas, self.postfit_model, set_pulse_nums = True)
        self.fitted = True
        ...
        self.selected_toas.table['delta_pulse_number'] = np.zeros(self.selected_toas.ntoas)
